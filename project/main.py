...
def main():
    ...
    simulation_amount = simulation["simulation_amount"]
    ...
    for number_of_simulations in range(simulation_amount):
        # Assign individual variables for intrusion parameters
        intrusion_center = None
        intrusion_radius = None
        if set_seed == True:
            np.random.seed(seed_number+number_of_simulations)
        # Initialize food grid
        food_grid = np.random.uniform(0.2, 0.8, grid_size)
        set_grid_size(grid_size)
        current_seed = seed_number+number_of_simulations
        current_name = "seed_" + str(current_seed) + "_intrusion_"+ str(intrusion["radius"])
        current_time = time.strftime("%Y%m%d-%H%M%S")
        result_folder_path = RESULT_PATH + current_name + "/"
        image_folder = "images"
        ...
        if isPlotResults:
            visualization.visualize(root_path=RESULT_PATH, folder_name=current_name)
